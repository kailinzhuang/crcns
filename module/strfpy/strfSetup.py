...
def srdata2strflab(srData, useRaw=False, preprocOptions={}):
    ...
    for k in range(pairCount):
        ...
        numTrials = min(numTrials, len(ds['resp']['rawSpikeIndicies'].flatten()[0].flatten()))
        totalStimLength += ds['stim'].flatten()['tfrep'][0].flatten()['spec'][0].shape[1]
        totalRespLength += len(ds['resp']['psth'].flatten()[0].flatten())
    ...
    for k in range(pairCount):
        ...
        stim = ds['stim'].flatten()['tfrep'][0].flatten()['spec'][0].T
        stimLen = stim.shape[0]
        resp = ds['resp']['psth'].flatten()[0].flatten()
