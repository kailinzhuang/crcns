...
def df_internal_cal_CrossCorr(stimval, psthval, twin, do_fourier=None):
    ...
    if do_fourier:
        for ib1 in range(nband):
            CSR_JN[ib1,:] = sps.correlate(stimval[ib1,:], psthval.flatten(), mode='same')[int(len(stimval[ib1, :])/2-twin):int(len(stimval[ib1, :])/2+twin+1)]
    else:
        pt = psthval.T
        for tid in range(-twin, twin+1):
            onevect = slice(max(1,tid+1)-1, min(N,N+tid)-1)
            othervect = slice(onevect.start-tid-1, onevect.stop-tid-1)
